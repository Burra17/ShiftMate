...
import { fetchShiftsPaginated, updateShift, deleteShift } from '../../api';
...
// Beräkna passtid i timmar från HH:MM-strängar
...
const ShiftListTab = ({ users }) => {
    ...
    const loadShifts = async (p = 1) => {
        setLoading(true);
        try {
            const data = await fetchShiftsPaginated({ page: p, pageSize });
    ...
    const handleDeleteShift = async (shiftId) => {
        const confirmed = await confirm({
            title: 'Radera pass',
            message: 'Är du säker på att du vill radera detta pass? Eventuella väntande bytesförfrågningar kommer avbrytas.',
            confirmLabel: 'Radera',
            variant: 'danger',
        });
        if (!confirmed) return;
